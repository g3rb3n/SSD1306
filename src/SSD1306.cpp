...
Page buffer LCDWIDTH x LCDHEIGHT divided by 8 = LCDSIZE bytes
...
All drawing function will first be drawn on this page buffer, only upon calling display() function will transfer the page buffer to the actual LCD controller's memory.
*/
static uint8_t screenmemory [LCDSIZE];
 ...
 */
SSD1306::SSD1306(ByteIO* _io):
  byteIO(_io),
  rstEnabled(false)
{
}

SSD1306::SSD1306(ByteIO* _io, uint8_t _rstPin):
  byteIO(_io),
  rstPin(_rstPin),
  rstEnabled(true) 
{
...
/** 
 * Initialisation of SSD1306 Library.
 */
void SSD1306::begin()
{
	// default 5x7 font
...
void SSD1306::clear(uint8_t mode) {
	...
	{
		memset(screenmemory,0,LCDSIZE);			// (64 x 48) / 8 = LCDSIZE
		//display();
	}
}
...
*/
uint8_t *SSD1306::getScreenBuffer(void) {
	return screenmemory;
}
...
*/
void SSD1306::drawBitmap(uint8_t * bitArray)
{
